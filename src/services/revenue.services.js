    ...
    async getListOrderByTime(startTime, endTime) {
        ...
        for (let i = 0; i < orders.length; i++) {
            ...
            for (let j = 0; j < order_items.length; j++) {
                originalPrice += order_items[j].Item.import_price * order_items[j].order_item_infor.quantity;
            }
    ...
    async getRevenueOfProduction(startTime, endTime) {
        ...
        for (let i = 0; i < sortedProducts.length; i++) {
            ...
            const image = await db.Image.findOne({
                where: { id_item: sortedProducts[i].id },
            });
            ...
            sortedProducts[i].Category = item.Category;
        }
        const totalRevenue = orders.reduce((acc, order) => acc + order.totalPrice, 0);
