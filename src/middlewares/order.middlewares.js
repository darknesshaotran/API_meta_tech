...
const checkItemOrderValidator = checkSchema(
    {
        Item: {
            custom: {
                options: async (value) => {
                    const { id_color_item, quantity, price } = value;
                    ...
                    if (Number(price) !== Number(item.Item.price)) {
            ...
            custom: {
                options: async (value) => {
                    for (let i = 0; i < value.length; i++) {
                        const { id_color_item, quantity, price, id_cartItem } = value[i];
